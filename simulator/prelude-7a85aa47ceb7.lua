...
-- which builds a real path instead of accumulating SVG path data. The simulator,
-- which has no native path type, loads the pure-Lua stand-in from prelude_drawing_path.lua
...
local function layoutResolvePivotAnchor2D(geometry, baseCenterX, baseCenterY, anchor, character)
    ...
    elseif anchor == "bottom_right" then
        targetX = targetX + halfWidth
        targetY = targetY - halfHeight
    elseif anchor == "baseline_left" and character then
        targetX = targetX - halfWidth
        targetY = geometry.canvas_origin_y
    elseif anchor == "baseline" and character then
        targetY = geometry.canvas_origin_y
    elseif anchor == "baseline_right" and character then
        targetX = targetX + halfWidth
        targetY = geometry.canvas_origin_y
    elseif anchor == "typesetting_origin" and character then
    ...
    elseif anchor ~= "center" then
        if anchor == "baseline_left"
            or anchor == "baseline"
            or anchor == "baseline_right"
            or anchor == "typesetting_origin"
...
function mt.bounce_ground(ctx, item, groundY, config)
    ...
    --   "baseline": aligns the character's horizontal baseline on groundY
...
---Position a queued item is measured from along the writing axis. Characters
---expose the origin the host laid them out at, which is what carries tracking
---and per-character margins; parts have no origin of their own, so their own
---centre stands in.
---@param item table
---@param isVertical boolean
---@return number
local function queueItemOrigin(item, isVertical)
    local geometry = item.geometry
    local result = 0.0
    if geometry == nil then
        result = 0.0
    elseif isVertical then
        result = geometry.vertical_origin_y
            or geometry.canvas_center_y
            or geometry.bounds_center_y
            or 0.0
    else
        result = geometry.canvas_origin_x
            or geometry.canvas_center_x
            or geometry.bounds_center_x
            or 0.0
    end
    return result
end

---Distributes characters (or parts) along an arc-length path so that they
---read correctly, and returns the normalized distance ratio each one occupies.
---
---This exists because hand-rolling the same logic means guaranteeing two
---things yourself, each of which fails silently -- a plausible-looking but
---wrong result rather than an error:
---
---  * Spacing by item INDEX (for example `mt.distribute`) ignores glyph
---    widths, so proportional text loses its letter spacing.
---  * Accumulating widths in index order while measuring back from the path
---    END reverses the reading order, because the accumulation direction and
---    the travel direction disagree.
---
---Both are avoided here by walking the targets in the order given and spacing
---them by each item's own natural advance.
---
---Spacing is measured from the ACTUAL laid-out origins (the same approach
---`mt.layout.reflow` takes), not from the bare glyph advance, so tracking,
---per-character margins and kerning that the host parameters contribute are
---all carried onto the path. `options.gap` is an extra offset on top of that,
---so a gap of 0 reproduces the host's own letter spacing along the curve.
---
---Slots ascend with distance along the TRAVEL direction, which is not the same
---as screen order: on a path travelling right to left, ascending distance means
---decreasing x, so ascending index order lays the text out mirrored on screen.
---Pass `options.targets` in reverse reading order to keep such a run readable
---(at the cost of the last item leading the queue instead of the first).
---
---The return value is a `slots` list indexed the same way as `ctx.chars` (or
---`ctx.parts`), where `slots[index]` is the distance ratio in 0-1 at which
---that item sits. It is a position, not an animation: feed it to
---`path:at_distance` directly to lay the text out along the path, or clamp a
---shared progress value with `math.min(progress, slots[index])` to make the
---items drive up the path and queue at their own slots.
---
---`options.align` places the assembled run on the path:
---  * `"end"` (default) puts the LAST reading position at the path end, so a
---    convoy arrives with its leader at the finish.
---  * `"start"` puts the first reading position at the path start.
---  * `"center"` centers the run on the path.
---`options.gap` adds extra spacing between items in canvas-width units, on top
---of the host's own letter spacing.
---`options.targets` restricts the layout to the listed indices (in reading
---order); other indices are left absent from the result.
---`options.mode` selects the axis the host laid the text out on:
---`"horizontal"` (default) or `"vertical"`.
---@param ctx table
---@param path MtArcLengthPath result of mt.path.arc_length
---@param options table|nil `{ align = "end"|"start"|"center", gap = number, mode = "horizontal"|"vertical", target_type = "character"|"part", targets = integer[] }`
---@return number[] slots distance ratio per item index
function mt.layout.queue_on_path(ctx, path, options)
    assert(ctx and ctx.canvas, "queue_on_path requires an OnLayout context")
    assert(path and path.length, "queue_on_path requires an mt.path.arc_length path")

    local kZeroLengthEpsilon = 1e-9
    local currentOptions = options or {}
    local alignment = currentOptions.align or "end"
    local extraGap = currentOptions.gap or 0.0
    local isPart = (currentOptions.target_type == "part")
    local isVertical = (currentOptions.mode == "vertical")
    local container = isPart and ctx.parts or ctx.chars
    local containerCount = isPart and ctx.part_count or ctx.char_count
    local aspectRatio = ctx.canvas.aspect_ratio
    local totalLength = path:length()

    local slots = {}
    if containerCount ~= nil and containerCount > 0 and totalLength > kZeroLengthEpsilon then
        -- Reading order: ascending index is the shaped reading order for both
        -- containers, so the run is built in that direction and only the
        -- ANCHORING is chosen by options.align. Measuring back from the path
        -- end must not also reverse the walk.
        local orderedIndices = currentOptions.targets
        if orderedIndices == nil then
            orderedIndices = {}
        ...
        -- Step from each item to the next by the distance the HOST actually laid
        -- them out with, so tracking / margins / kerning survive the transfer
        -- onto the path. The bare glyph advance would drop all of them, and
        -- reading host parameters directly would miss shaper-applied kerning.
        local steps = {}
        local runLength = 0.0
        for order = 1, #orderedIndices - 1 do
            local current = container[orderedIndices[order]]
            local following = container[orderedIndices[order + 1]]
            local step = 0.0
            if current ~= nil and following ~= nil then
                local currentOrigin = queueItemOrigin(current, isVertical)
                local followingOrigin = queueItemOrigin(following, isVertical)
                -- The magnitude of the gap the host left between the two, not
                -- its sign: `targets` may walk the text backwards (that is how
                -- a reversed path keeps its reading order), and a signed step
                -- would collapse every item in such a run onto one slot.
                local originGap = math.abs(followingOrigin - currentOrigin)
                if isVertical then
                    step = originGap + extraGap
                else
                    step = (originGap + extraGap) * aspectRatio
                end
            end
            steps[order] = step
        ...
        local runStart = totalLength - runLength
        if alignment == "start" then
            runStart = 0.0
        elseif alignment == "center" then
            runStart = (totalLength - runLength) * 0.5
        end

        -- Each item sits ON its own slot (the origin-to-origin step already
        -- carries the clearance between neighbours), so the cursor advances by
        -- the step to the next item rather than by this item's own width.
        local cursor = runStart
        for order, index in ipairs(orderedIndices) do
            slots[index] = mt.saturate(cursor / totalLength)
            cursor = cursor + (steps[order] or 0.0)
        end
    end
...
---------------------------------------------------------------------------
-- Arc-length parameterized paths (mt.path.arc_length)
--
-- mt.path.bezier / catmull_rom are parameterized by the curve parameter t,
-- and equal steps in t are NOT equal distances along a curve: samples bunch
-- up wherever the curve bends. Anything that spaces items BY DISTANCE (a
-- convoy along a route, letters queued on a path, a constant-speed traversal)
-- therefore needs a cumulative length table. This builds that table once and
-- exposes distance-based lookup, so callers stop reimplementing it.
--
-- Distances are aspect-corrected at build time: x deltas are multiplied by
-- the canvas aspect ratio so that a distance means the same thing
-- horizontally and vertically on a non-square canvas. Ratios handed to
-- at_distance are therefore in canvas-height units internally, but callers
-- only ever see normalized 0-1 ratios.
---------------------------------------------------------------------------

-- Captured before the sandbox strips the global (see the storage section).
local setPathMetatable = setmetatable

local kArcSampleCount = 64
local kArcEpsilon = 1e-9

---An arc-length parameterized path returned by mt.path.arc_length. The
---trailing-underscore fields are internal state; call the methods instead.
---@class MtArcLengthPath
---@field cumulativeLengths_ number[]
---@field totalLength_ number
---@field sample_ fun(t: number): number, number, number, number
---@field aspectRatio_ number
local ArcLengthPath = {}
ArcLengthPath.__index = ArcLengthPath

---Total path length in aspect-corrected canvas units.
---@return number
function ArcLengthPath:length()
    return self.totalLength_
end

---Position and heading at a normalized distance along the path.
---`distanceRatio` 0 is the path start and 1 is the path end, measured by
---distance rather than by curve parameter, so equal ratio steps are equal
---distances on screen. Out-of-range ratios clamp to the endpoints.
---
---The third return value is a heading in DEGREES, already converted to the
---screen convention used by `rotation` fields: canvas Y grows downward, so
---the raw tangent Y is negated here. Callers assigning to `rotation` can use
---it directly instead of rederiving the sign.
---@param distanceRatio number
---@return number x, number y, number headingDegrees
function ArcLengthPath:at_distance(distanceRatio)
    local targetDistance = mt.saturate(distanceRatio) * self.totalLength_
    local cumulativeLengths = self.cumulativeLengths_
    local curveT = 1.0
    local resolved = false

    for step = 1, kArcSampleCount do
        local segmentEnd = cumulativeLengths[step + 1]
        if not resolved and segmentEnd >= targetDistance then
            local segmentStart = cumulativeLengths[step]
            local segmentLength = segmentEnd - segmentStart
            local withinSegment = 0.0
            if segmentLength > kArcEpsilon then
                withinSegment = (targetDistance - segmentStart) / segmentLength
            end
            curveT = (step - 1 + withinSegment) / kArcSampleCount
            resolved = true
        end
    end

    local x, y, tangentX, tangentY = self.sample_(curveT)
    local headingDegrees = math.deg(math.atan(-tangentY, tangentX))
    return x, y, headingDegrees
end

---Builds an arc-length parameterized path from control points.
---
---`points` is a list of `{ x, y }` in whatever coordinate system the caller
---uses; canvas-normalized points are the common case and are what
---`mt.layout.queue_on_path` and `mt.layout.place_2d` expect. With 2 or more
---points the curve is the Catmull-Rom spline through all of them (it passes
---exactly through every point); pass exactly 4 points with
---`kind = "bezier"` to treat them as cubic Bezier control points instead.
---
---`aspectRatio` is `ctx.canvas.aspect_ratio`. It makes a distance mean the
---same thing on both axes; omitting it (or passing nil) measures in raw
---normalized units, which stretches spacing on a non-square canvas.
---@param points MtPathPoint[]
---@param aspectRatio number|nil
---@param options table|nil `{ kind = "catmull_rom"|"bezier" }`
---@return MtArcLengthPath
function mt.path.arc_length(points, aspectRatio, options)
    local pointCount = #points
    if pointCount < 2 then
        error("mt.path.arc_length: points must contain at least 2 points", 2)
    end

    local kind = "catmull_rom"
    if options ~= nil and options.kind ~= nil then
        kind = options.kind
    end
    if kind ~= "catmull_rom" and kind ~= "bezier" then
        error('mt.path.arc_length: kind must be "catmull_rom" or "bezier"', 2)
    end
    if kind == "bezier" and pointCount ~= 4 then
        error("mt.path.arc_length: bezier requires exactly 4 points", 2)
    end

    local sample
    if kind == "bezier" then
        sample = function(t)
            return mt.path.bezier(points[1], points[2], points[3], points[4], t)
        end
    else
        sample = function(t)
            return mt.path.catmull_rom(points, t)
        end
    end

    local correctedAspect = aspectRatio or 1.0
    local cumulativeLengths = { 0.0 }
    local previousX, previousY = sample(0.0)
    local totalLength = 0.0

    for step = 1, kArcSampleCount do
        local sampleX, sampleY = sample(step / kArcSampleCount)
        local deltaX = (sampleX - previousX) * correctedAspect
        local deltaY = sampleY - previousY
        totalLength = totalLength + math.sqrt(deltaX * deltaX + deltaY * deltaY)
        cumulativeLengths[step + 1] = totalLength
        previousX, previousY = sampleX, sampleY
    end

    return setPathMetatable({
        cumulativeLengths_ = cumulativeLengths,
        totalLength_ = totalLength,
        sample_ = sample,
        aspectRatio_ = correctedAspect,
    }, ArcLengthPath)
end
