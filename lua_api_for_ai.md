...
- Rule on debugging with `print(...)`:
  ...
  3. In that case, ask the user to mark the range on the timeline (if narrowing down is needed), click the Frame Dump button (bug icon), and share the log output. The dumped log contains a `=== SIMULATOR CONTEXT ===` header (canvas size, FPS, text, global Inspector settings, Script Inputs, character topology) followed by `=== FRAME LOGS ===`.
